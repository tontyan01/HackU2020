...
namespace FormsSample
{
	public class Charts : Application
	{
		...
		async void CreateScene ()
		{
			...
			//mybagの大きさは多分basenode
			baseNode.Scale = new Vector3(size1 * 1.5f, 1, size2 * 1.5f);
			bars = new List<Bar>(size1 * size2);
			for (var i = 0f; i < size1 * 1.5f; i += 1.5f)
			{
					var boxNode = plotNode.CreateChild();
					boxNode.Position = new Vector3(size1 / 2f - i, 0, size2 / 2f - 10);
					var box = new Bar(new Color(RandomHelper.NextRandom(), RandomHelper.NextRandom(), RandomHelper.NextRandom(), 0.9f));
					boxNode.AddComponent(box);
					//box.Scale(x,z,y);
					box.SetValueWithAnimation(1,2,3);
	...
	public class Bar : Component
	{
		...
		public void SetValueWithAnimation(float x,float y,float z) => barNode.RunActionsAsync(new EaseBackOut(new ScaleTo(3f, x, y, z)));
		...
		public override void OnAttachedToNode(Node node)
		{
			...
			barNode.Scale = new Vector3(20, 0, 1); //means zero height
